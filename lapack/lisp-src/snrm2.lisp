...
(let* ((one 1.0) (zero 0.0))
  (declare (type (single-float 1.0 1.0) one)
           ...
           (ignorable one zero))
  (defun snrm2 (n x incx)
    (declare (type (array single-float (*)) x)
             (type (f2cl-lib:integer4) incx n))
    (f2cl-lib:with-multi-array-data
        ((x single-float x-%data% x-%offset%))
      (prog ((ix 0) (absxi 0.0) (norm 0.0) (scale 0.0) (ssq 0.0) (snrm2 0.0))
        (declare (type (f2cl-lib:integer4) ix)
                 (type (single-float) absxi norm scale ssq snrm2))
        ...
        (go end_label)
       end_label
        (return (values snrm2 nil nil nil))))))

(in-package #-gcl #:cl-user #+gcl "CL-USER")
#+#.(cl:if (cl:find-package '#:f2cl) '(and) '(or))
(eval-when (:load-toplevel :compile-toplevel :execute)
  (setf (gethash 'fortran-to-lisp::snrm2 fortran-to-lisp::*f2cl-function-info*)
