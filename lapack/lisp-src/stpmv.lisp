...
(let* ((zero 0.0))
  (declare (type (single-float 0.0 0.0) zero)
           (ignorable zero))
  (defun stpmv (uplo trans diag n ap x incx)
    (declare (type (array single-float (*)) x ap)
             (type (f2cl-lib:integer4) incx n)
             (type (string 1) diag trans uplo))
    (f2cl-lib:with-multi-array-data
      ...
      (prog ((nounit nil) (i 0) (info 0) (ix 0) (j 0) (jx 0) (k 0) (kk 0)
        ...
        (cond ((/= info 0) (xerbla "STPMV " info) (go end_label)))
        (if (= n 0)
            (go end_label))
...
(eval-when (:load-toplevel :compile-toplevel :execute)
  (setf (gethash 'fortran-to-lisp::stpmv fortran-to-lisp::*f2cl-function-info*)
