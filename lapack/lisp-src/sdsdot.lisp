...
(defun sdsdot (n sb sx incx sy incy)
  (declare (type (array single-float (*)) sy sx)
           (type (single-float) sb)
           (type (f2cl-lib:integer4) incy incx n))
  (f2cl-lib:with-multi-array-data
      ((sx single-float sx-%data% sx-%offset%)
       (sy single-float sy-%data% sy-%offset%))
    (prog ((i 0) (kx 0) (ky 0) (ns 0) (dsdot 0.0d0) (sdsdot 0.0))
      (declare (type (single-float) sdsdot)
               (type (double-float) dsdot)
               (type (f2cl-lib:integer4) ns ky kx i))
      (setf dsdot (coerce sb 'double-float))
      (cond
       ((<= n 0) (setf sdsdot (coerce dsdot 'single-float)) (go end_label)))
      (cond
       ...
       (t (setf kx 1) (setf ky 1)
        ...
        (f2cl-lib:fdo (i 1 (f2cl-lib:int-add i 1))
          ...
          (tagbody
           ...
           label100001))))
      (setf sdsdot (coerce dsdot 'single-float))
      (go end_label)
     end_label
      (return (values sdsdot nil nil nil nil nil nil)))))

(in-package #-gcl #:cl-user #+gcl "CL-USER")
#+#.(cl:if (cl:find-package '#:f2cl) '(and) '(or))
(eval-when (:load-toplevel :compile-toplevel :execute)
  (setf (gethash 'fortran-to-lisp::sdsdot
