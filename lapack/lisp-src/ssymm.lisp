...
(let* ((one 1.0) (zero 0.0))
  (declare (type (single-float 1.0 1.0) one)
           ...
           (ignorable one zero))
  (defun ssymm (side uplo m n alpha a lda b ldb$ beta c ldc)
    (declare (type (array single-float (*)) c b a)
             (type (single-float) beta alpha)
             (type (f2cl-lib:integer4) ldc ldb$ lda n m)
             (type (string 1) uplo side))
    (f2cl-lib:with-multi-array-data
      ...
      (prog ((upper nil) (i 0) (info 0) (j 0) (k 0) (nrowa 0) (temp1 0.0)
             (temp2 0.0))
        (declare (type f2cl-lib:logical upper)
                 (type (f2cl-lib:integer4) i info j k nrowa)
                 (type (single-float) temp1 temp2))
        ...
        (cond
         ...
         ((< ldc (max (the f2cl-lib:integer4 1) (the f2cl-lib:integer4 m)))
          (setf info 12)))
        (cond ((/= info 0) (xerbla "SSYMM " info) (go end_label)))
...
(eval-when (:load-toplevel :compile-toplevel :execute)
  (setf (gethash 'fortran-to-lisp::ssymm fortran-to-lisp::*f2cl-function-info*)
