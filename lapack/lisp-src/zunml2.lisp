...
(let* ((one (f2cl-lib:cmplx 1.0d0 0.0d0)))
  (declare (type (f2cl-lib:complex16) one)
           (ignorable one))
  (defun zunml2 (side trans m n k a lda tau c ldc work info)
      ...
      (prog ((aii #C(0.0d0 0.0d0)) (taui #C(0.0d0 0.0d0)) (i 0) (i1 0) (i2 0)
        ...
        (cond ((and (not left) (not (lsame side "R"))) (setf info -1))
              ...
              ((< ldc
               ...
               (setf info -10)))
        (cond
         ((/= info 0) (xerbla "ZUNML2" (f2cl-lib:int-sub info))
          (go end_label)))
        (if (or (= m 0) (= n 0) (= k 0))
            (go end_label))
        (cond
...
(eval-when (:load-toplevel :compile-toplevel :execute)
  (setf (gethash 'fortran-to-lisp::zunml2
