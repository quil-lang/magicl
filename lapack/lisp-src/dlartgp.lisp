...
(let* ((zero 0.0d0) (one 1.0d0) (two 2.0d0))
  ...
  (defun dlartgp (f g cs sn r)
    (declare (type (double-float) r sn cs g f))
    (prog ((eps 0.0d0) (f1 0.0d0) (g1 0.0d0) (safmin 0.0d0) (safmn2 0.0d0)
           (safmx2 0.0d0) (scale 0.0d0) (i 0) (count$ 0))
      (declare (type (double-float) eps f1 g1 safmin safmn2 safmx2 scale)
      ...
      (cond
       ((= g zero) (setf cs (f2cl-lib:sign one f)) (setf sn zero)
        (setf r (abs f)))
       ((= f zero) (setf cs zero) (setf sn (f2cl-lib:sign one g))
        (setf r (abs g)))
       (t (setf f1 f) (setf g1 g) (setf scale (max (abs f1) (abs g1)))
        ...
        (cond ((< r zero) (setf cs (- cs)) (setf sn (- sn)) (setf r (- r))))))
      (go end_label)
     end_label
...
(eval-when (:load-toplevel :compile-toplevel :execute)
  (setf (gethash 'fortran-to-lisp::dlartgp
