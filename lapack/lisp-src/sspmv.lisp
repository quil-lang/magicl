...
(let* ((one 1.0) (zero 0.0))
  (declare (type (single-float 1.0 1.0) one)
           ...
           (ignorable one zero))
  (defun sspmv (uplo n alpha ap x incx beta y incy)
    (declare (type (array single-float (*)) y x ap)
             (type (single-float) beta alpha)
             (type (f2cl-lib:integer4) incy incx n)
             (type (string 1) uplo))
    (f2cl-lib:with-multi-array-data
        ((uplo character uplo-%data% uplo-%offset%)
         (ap single-float ap-%data% ap-%offset%)
      ...
      (prog ((i 0) (info 0) (ix 0) (iy 0) (j 0) (jx 0) (jy 0) (k 0) (kk 0)
        ...
        (cond
         ...
         ((= incy 0) (setf info 9)))
        (cond ((/= info 0) (xerbla "SSPMV " info) (go end_label)))
...
(eval-when (:load-toplevel :compile-toplevel :execute)
  (setf (gethash 'fortran-to-lisp::sspmv fortran-to-lisp::*f2cl-function-info*)
