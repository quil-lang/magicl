...
(defun cdotu (n cx incx cy incy)
  ...
  (f2cl-lib:with-multi-array-data
    ...
    (prog ((i 0) (ix 0) (iy 0) (ctemp #C(0.0 0.0)) (cdotu #C(0.0 0.0)))
      (declare (type (complex) cdotu)
               (type (f2cl-lib:complex8) ctemp)
               (type (f2cl-lib:integer4) iy ix i))
      (setf ctemp (f2cl-lib:cmplx 0.0 0.0))
      (setf cdotu (f2cl-lib:cmplx 0.0 0.0))
      (if (<= n 0)
          (go end_label))
      (cond
       ((and (= incx 1) (= incy 1))
      ...
      (go end_label)
     end_label
      (return (values cdotu nil nil nil nil nil)))))

(in-package #-gcl #:cl-user #+gcl "CL-USER")
#+#.(cl:if (cl:find-package '#:f2cl) '(and) '(or))
(eval-when (:load-toplevel :compile-toplevel :execute)
  (setf (gethash 'fortran-to-lisp::cdotu fortran-to-lisp::*f2cl-function-info*)
