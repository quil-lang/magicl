...
(defmethod lapack-eig ((m matrix/complex-double-float))
  ...
  (let ((rows (nrows m))
    ...
    (let ((jobvl "N")
          (jobvr "V")
          (a (storage a-tensor))
          (w (make-array rows :element-type '(complex double-float)))
          (vl (make-array rows :element-type '(complex double-float)))
          (vr (make-array (* rows rows) :element-type '(complex double-float)))
          (lwork -1)
          (rwork (make-array (* 2 rows) :element-type 'double-float))
          (info 0))
      (let ((work (make-array (max 1 lwork) :element-type '(complex double-float))))
        ...
        (magicl.lapack-cffi::%zgeev jobvl jobvr rows a rows w vl 1 vr rows work lwork rwork info)
        (values (coerce w 'list) (from-array vr (list rows cols) :order :column-major))))))

(defmethod lapack-lu ((matrix matrix/complex-double-float))
...
(defmethod lapack-svd ((m matrix/complex-double-float))
  ...
  (let ((jobu "A")
        ...
        (info 0))
    (let ((lda rows)
          (s (make-array (min rows cols) :element-type 'double-float))
          (ldu rows)
          (ldvt cols)
          (work1 (make-array (max 1 lwork) :element-type '(complex double-float)))
          (work nil)
          (rwork (make-array (* 5 (min rows cols)) :element-type 'double-float)))
      (let ((u (make-array (* ldu rows) :element-type '(complex double-float)))
            (vt (make-array (* ldvt cols) :element-type '(complex double-float))))
        ;; run it once as a workspace query
        ...
        (let ((smat (make-array (* rows cols) :element-type 'double-float)))
...
(defmethod lapack-ql ((m matrix/complex-double-float))
  (let* ((rows (nrows m))
         (cols (ncols m))
         (a-tensor (deep-copy-tensor m))
         (a (storage a-tensor))
    ...
    (when (eql :row-major (order m))
      (transpose! a-tensor))
    (let ((lda rows)
          (tau (make-array (min rows cols) :element-type '(complex double-float)))
          (work (make-array (max 1 lwork) :element-type '(complex double-float))))
      ;; run it once as a workspace query
      (magicl.lapack-cffi::%zgeqlf rows cols a lda tau work lwork info)
...
(defmethod lapack-qr ((m matrix/complex-double-float))
  (let* ((rows (nrows m))
         (cols (ncols m))
         (a-tensor (deep-copy-tensor m))
         (a (storage a-tensor))
...
(defmethod lapack-rq ((m matrix/complex-double-float))
  (let* ((rows (nrows m))
         (cols (ncols m))
         (a-tensor (deep-copy-tensor m))
         (a (storage a-tensor))
    ...
    (let ((lda rows)
          ...
          (work (make-array (max 1 lwork) :element-type '(complex double-float))))
      ;; run it once as a workspace query
      (magicl.lapack-cffi::%zgerqf rows cols a lda tau work lwork info)
      (setf lwork (round (realpart (aref work 0))))
      (setf work (make-array (max 1 lwork) :element-type '(complex double-float)))
      ;; run it again with optimal workspace size
      (magicl.lapack-cffi::%zgerqf rows cols a lda tau work lwork info)
...
(defmethod lapack-lq ((m matrix/complex-double-float))
  (let* ((rows (nrows m))
         (cols (ncols m))
         (a-tensor (deep-copy-tensor m))
         (a (storage a-tensor))
    ...
    (let ((lda rows)
          ...
          (work (make-array (max 1 lwork) :element-type '(complex double-float))))
      ;; run it once as a workspace query
      (magicl.lapack-cffi::%zgelqf rows cols a lda tau work lwork info)
      (setf lwork (round (realpart (aref work 0))))
      (setf work (make-array (max 1 lwork) :element-type '(complex double-float)))
      ;; run it again with optimal workspace size
      (magicl.lapack-cffi::%zgelqf rows cols a lda tau work lwork info)
...
(defmethod lapack-ql-q ((m matrix/complex-double-float) tau)
  (let ((m (nrows m))
        (n (ncols m))
        (a (storage m))
        (k (size tau))
    ...
    (let ((lda m)
          (work (make-array (max 1 lwork) :element-type '(complex double-float))))
      ;; run it once as a workspace query
      (magicl.lapack-cffi::%zungql m n k a lda atau work lwork info)
      (setf lwork (round (realpart (aref work 0))))
      (setf work (make-array (max 1 lwork) :element-type '(complex double-float)))
      ;; run it again with optimal workspace size
      (magicl.lapack-cffi::%zungql m n k a lda atau work lwork info)
...
(defmethod lapack-qr-q ((m matrix/complex-double-float) tau)
  (let ((m (nrows m))
        (n (ncols m))
        (a (storage m))
        (k (size tau))
    ...
    (let ((lda m)
          (work (make-array (max 1 lwork) :element-type '(complex double-float))))
      ;; run it once as a workspace query
      (magicl.lapack-cffi::%zungqr m n k a lda atau work lwork info)
      (setf lwork (round (realpart (aref work 0))))
      (setf work (make-array (max 1 lwork) :element-type '(complex double-float)))
      ;; run it again with optimal workspace size
      (magicl.lapack-cffi::%zungqr m n k a lda atau work lwork info)
...
(defmethod lapack-rq-q ((m matrix/complex-double-float) tau)
  (let ((m (nrows m))
        (n (ncols m))
        (a (storage m))
        (k (size tau))
    ...
    (let ((lda m)
          (work (make-array (max 1 lwork) :element-type '(complex double-float))))
      ;; run it once as a workspace query
      (magicl.lapack-cffi::%zungrq m n k a lda atau work lwork info)
      (setf lwork (round (realpart (aref work 0))))
      (setf work (make-array (max 1 lwork) :element-type '(complex double-float)))
      ;; run it again with optimal workspace size
      (magicl.lapack-cffi::%zungrq m n k a lda atau work lwork info)
...
(defmethod lapack-lq-q ((m matrix/complex-double-float) tau)
  (let ((m (nrows m))
        (n (ncols m))
        (a (storage m))
        (k (size tau))
    ...
    (let ((lda m)
          (work (make-array (max 1 lwork) :element-type '(complex double-float))))
      ;; run it once as a workspace query
      (magicl.lapack-cffi::%zunglq m n k a lda atau work lwork info)
      (setf lwork (round (realpart (aref work 0))))
      (setf work (make-array (max 1 lwork) :element-type '(complex double-float)))
      ;; run it again with optimal workspace size
      (magicl.lapack-cffi::%zunglq m n k a lda atau work lwork info)
