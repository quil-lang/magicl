...
(defun svd (m &key reduced)
  ...
  (let* ((jobu (if reduced "S" "A"))
    ...
    (let ((smat (make-D-storage (* u-cols vt-rows))))
      (dotimes (i k)
        (setf (aref smat (column-major-index u-cols i i))
              (aref s i)))
      (values (make-matrix :rows rows :cols u-cols :data u)
              (make-matrix :rows u-cols :cols vt-rows :data smat)
              (make-matrix :rows vt-rows :cols cols :data vt)))))
