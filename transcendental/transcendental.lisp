...
(defun expm (m)
  ...
  (let ((ideg 6)
        ...
        (h-tensor (magicl::deep-copy-tensor m))
        (iexph 0)
        (ns 0)
        (iflag 0))
    (when (eql :row-major (magicl::layout h-tensor)) (magicl:transpose! h-tensor))
    (let ((h (magicl::storage h-tensor))
          (lwsp (+ (* 4 rows rows) ideg 1))
